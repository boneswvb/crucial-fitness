...
function PatientDetailsCon() {
  ...
  const { email, fullname, lastname, dateofbirth, adress, cellno, econtact, ecellno, relationship } = formState

  const handleSubmitPatDetails = async (e) => {
    e.preventDefault();
    try {
      const date = Date().slice(8, 10) + Date().slice(3, 7) + Date().slice(10, 15)
      const body = { email, fullname, lastname, dateofbirth, adress, cellno, econtact, ecellno, relationship, date }
